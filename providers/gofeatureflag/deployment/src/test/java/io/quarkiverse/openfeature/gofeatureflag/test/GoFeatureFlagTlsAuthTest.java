...
@Certificates(baseDir = "target/certs", certificates = @Certificate(name = "goff-tls-auth", formats = Format.PEM))
public class GoFeatureFlagTlsAuthTest {
    private static final GoFeatureFlagTestContainer goff = new GoFeatureFlagTestContainer("goff-proxy-auth.yaml");

    @RegisterExtension
    @Order(1)
    static final TlsProxyExtension proxy = TlsProxyExtension.create("target/certs/goff-tls-auth.key",
            "target/certs/goff-tls-auth.crt", goff, GoFeatureFlagTestContainer.HTTP_PORT);
    ...
    static {
        test.setBeforeAllCustomizer(() -> {
            test.overrideConfigKey("quarkus.tls.goff.trust-store.pem.certs",
                    new File("target/certs/goff-tls-auth-ca.crt").getAbsolutePath());
