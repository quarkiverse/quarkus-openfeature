...
@Certificates(baseDir = "target/certs", certificates = {
        @Certificate(name = "unleash-good-tls", formats = Format.PEM),
        @Certificate(name = "unleash-bad-tls", formats = Format.PEM)
})
public class UnleashTlsBadTest {
    private static final UnleashTestContainer unleash = new UnleashTestContainer("unleash.json");

    @RegisterExtension
    @Order(1)
    static final TlsProxyExtension proxy = TlsProxyExtension.create("target/certs/unleash-good-tls.key",
            "target/certs/unleash-good-tls.crt", unleash, UnleashTestContainer.HTTP_PORT);
    ...
    static {
        test.setBeforeAllCustomizer(() -> {
            test.overrideConfigKey("quarkus.tls.unleash.trust-store.pem.certs", "target/certs/unleash-bad-tls-ca.crt");
