...
@Certificates(baseDir = "target/certs", certificates = @Certificate(name = "flipt-tls", formats = Format.PEM))
public class FliptTlsTest {
    private static final FliptTestContainer flipt = new FliptTestContainer.Builder("flipt-tls-config.yml")
            .withTls("target/certs/flipt-tls.key", "target/certs/flipt-tls.crt")
            .build();
    ...
    static {
        test.setBeforeAllCustomizer(() -> {
            ...
            FliptImporter.run(url, features, null, new WebClientOptions()
                    .setSsl(true)
                    .setTrustAll(true)
                    .setVerifyHost(false));

            test.overrideConfigKey("quarkus.tls.flipt.trust-store.pem.certs", "target/certs/flipt-tls-ca.crt");
